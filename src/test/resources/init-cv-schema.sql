...
-- 7. 子帳戶利率表 (QIRAT) - Sub Account Interest Rate
CREATE TABLE QIRAT (
    SUB_ACNT_PLAN_CODE  VARCHAR2(4)    NOT NULL,
    INT_RATE_TYPE       VARCHAR2(2)    NOT NULL,
    INT_RATE_DATE_STR   DATE           NOT NULL,
    INT_RATE_DATE_END   DATE,
    INT_RATE            NUMBER(7,4),
    CONSTRAINT PK_QIRAT PRIMARY KEY (SUB_ACNT_PLAN_CODE, INT_RATE_TYPE, INT_RATE_DATE_STR)
);

CREATE INDEX IDX_QIRAT_PLAN_TYPE ON QIRAT(SUB_ACNT_PLAN_CODE, INT_RATE_TYPE);

-- 8. 費率表 (PRAT) - Plan Rate
CREATE TABLE PRAT (
